...
void AAN(){
  ...
  if (Iteration == 1) {
    ...
    if (FetchFrame(prevFrame, Iteration - 1, FrameCounter)) {
        error_received = prevFrame.targetA - prevFrame.currentA;
        T_ff_prev = prevFrame.t_ff;
    } else {
      error_received = 0;
      T_ff_prev = 0;
    }
...
void RestStep(){
  ...
  if (micros() >= RestEndTime) {
      ...
      char filename[32];
      sprintf(filename, "AAN_%04d.bin", (unsigned long)Iteration);
