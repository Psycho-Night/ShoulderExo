...
// AAN controller - Maxon Current controller v4.0 Written for Teensy 4.1
// Soft start, Time in micro seconds 1 kHz system,
// Additional GUI communication 
...
float alpha = 0.006;  // working 0.006
const float beta = 0.0;  // Keep it 0.05


// Tune those and keep changes
float a = 1.5;      // for now 1.5 but might have to be decreased
...
float frequency = 0.05;             // Frequency in Hz (adjust as needed)
float amplitude = 35.0;             // Amplitude (max deviation from midpoint)
float offset = 35.0;                // Offset/midpoint of the sine wave
const long InitialisationDelay = 3000000; // Delay used for initialisation
float phaseShift = -PI/2;           // Phase shift for graduate raise from 0 deg -PI/2 if sin wave and 0 if step
...
void processCommand(String cmd) {
  ...
  if (cmd.startsWith("SET")) {
    // Example: SET 0.1 40 -1.57
    sscanf(cmd.c_str(), "SET %f %f %f %f", &frequency, &amplitude, &offset, &phaseShift);
    ...
    sscanf(cmd.c_str(), "TUNE %f %f", &a, &alpha);
